    ...
    if not pyinstaller_found:
        ...
        try:
            ...
            try:
                ...
                print(
                    "WARNING: PyInstaller module still not available after installation."
                )
            ...
            print(
                "Please try manually installing PyInstaller with: pip install pyinstaller"
            )
...
a = Analysis(['ai_assessor_main.py'],
             pathex=[],
             binaries=[],
             datas=[('ai_assessor/__init__.py', 'ai_assessor/__init__.py', 'DATA')],
             hiddenimports=[],
             hookspath=[],
             hooksconfig={},
             runtime_hooks=[],
...
"""
    ...
    try:
        ...
        try:
            print(
                f"Running command: {sys.executable} -m PyInstaller --clean ai_assessor.spec"
            )
        ...
        if not pyinstaller_success:
            ...
            if pyinstaller_cmd:
                try:
                    print(
                        f"Running command: {pyinstaller_cmd} --clean ai_assessor.spec"
                    )
                    result = subprocess.run(
        ...
        if not pyinstaller_success and sys.platform.startswith("win"):
            ...
            if os.path.exists(pyinstaller_exe):
                try:
                    print(
                        f"Running command: {pyinstaller_exe} --clean ai_assessor.spec"
                    )
                    result = subprocess.run(
        ...
        if not pyinstaller_success:
            ...
            print(
                "Please try manually running: python -m PyInstaller --clean ai_assessor.spec"
            )
            sys.exit(1)
    ...
    if os.path.exists(submissions_src):
        for file in os.listdir(submissions_src):
            if file.endswith(".docx"):
                shutil.copy2(
                    os.path.join(submissions_src, file),
                    os.path.join(submissions_dest, file),
                )
    ...
    if os.path.exists(support_src):
        for file in os.listdir(support_src):
            if file.endswith(".docx"):
                shutil.copy2(
                    os.path.join(support_src, file), os.path.join(support_dest, file)
                )

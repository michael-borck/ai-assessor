...
class SplashScreen:
    ...
    def __init__(self, root):
        ...
        try:
            ...
            logo_label = tk.Label(
                self.splash_root, image=self.logo, background="#f0f0f0"
            )
        ...
        self.progress = ttk.Progressbar(
            self.splash_root, orient="horizontal", length=400, mode="determinate"
        )
...
class AIAssessorGUI:
    ...
    def initialize_app(self):
        ...
        try:
            # Try to load logo if available
            self.root.iconphoto(
                False, PhotoImage(file="ai_assessor/ui/assets/icon.png")
            )
        except Exception:
            pass
        ...
        self.string_vars = {
            ...
            "user_prompt_path": tk.StringVar(
                value=self.config_manager.get_value("Paths", "UserPromptPath", "")
            ),
            "support_folder": tk.StringVar(
                value=self.config_manager.get_value("Paths", "SupportFolder", "")
            ),
            "submissions_folder": tk.StringVar(
                value=self.config_manager.get_value("Paths", "SubmissionsFolder", "")
            ),
            "output_folder": tk.StringVar(
                value=self.config_manager.get_value("Paths", "OutputFolder", "")
            ),
            "model": tk.StringVar(
            ...
            "temperature": tk.StringVar(
                value=self.config_manager.get_value("API", "Temperature", "0.7")
            ),
        }
    ...
    def update_api_client_settings(self):
        ...
        logging.debug(
            f"AIAssessorGUI.update_api_client_settings: api_key={api_key}, base_url={base_url}, ssl_verify={ssl_verify}"
        )
    ...
    def fetch_available_models(self, api_key):
        ...
        try:
            ...
            client = OpenAI(
                api_key=api_key, timeout=httpx.Timeout(10.0)  # 10 second timeout
            )
    ...
    def setup_ui(self):
        ...
        self.grading_tab = GradingView(
            self.tab_control, self.assessor, self.config_manager, self.string_vars
        )
    ...
    def create_menu(self):
        ...
        help_menu.add_command(
            label="View Documentation", command=self.show_documentation
        )
    ...
    def create_status_bar(self):
        ...
        self.api_status_var.set(
            "Not configured" if not self.string_vars["api_key"].get() else "Ready"
        )

        self.api_status_color = (
            "red" if not self.string_vars["api_key"].get() else "green"
        )
    ...
    def show_documentation(self):
        ...
        ttk.Label(
            content_frame,
            text="AI Assessor Documentation",
...
For issues or feature requests, please contact your system administrator.
        ...
        # Close button
        ttk.Button(content_frame, text="Close", command=doc_window.destroy).pack(
            pady=10
        )
...
© 2023-2025 All Rights Reserved
    ...
    def on_closing(self):
        ...
        self.config_manager.set_value(
            "Paths", "UserPromptPath", self.string_vars["user_prompt_path"].get()
        )
        self.config_manager.set_value(
            "Paths", "SupportFolder", self.string_vars["support_folder"].get()
        )
        self.config_manager.set_value(
            "Paths", "SubmissionsFolder", self.string_vars["submissions_folder"].get()
        )
        self.config_manager.set_value(
            "Paths", "OutputFolder", self.string_vars["output_folder"].get()
        )
        self.config_manager.set_value(

...
class GradingView(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # File list frame
        ttk.Label(self, text="Submissions:").grid(
        ...
        ttk.Button(
            button_frame, text="Grade Selected", command=self.grade_selected
        ).pack(side="left", padx=5, pady=5)
        ttk.Button(button_frame, text="Grade All", command=self.grade_all).pack(
            side="left", padx=5, pady=5
        )
        ttk.Button(
            button_frame, text="Refresh List", command=self.update_file_list
        ).pack(side="left", padx=5, pady=5)

        # Content display
        ttk.Label(self, text="Submission Content:").grid(
        ...
        ttk.Label(self, text="Feedback:").grid(
    ...
    def update_file_list(self):
        ...
        if (
            submissions_folder
            and os.path.exists(submissions_folder)
            and os.path.isdir(submissions_folder)
        ):
            try:
                # Get all docx files
            ...
            if os.path.exists(feedback_path):
                try:
                    ...
                    self.feedback_display.insert(
                        tk.END, f"Error reading feedback: {str(e)}"
                    )
            else:
                self.feedback_display.insert(
                    tk.END,
                    "Feedback not available. Use 'Grade Selected' to assess this submission.",
                )
    ...
    def grade_selected(self):
        ...
        if not selected_indices:
            messagebox.showwarning(
                "Warning", "Please select at least one submission to grade."
            )
            return
        ...
        ttk.Label(frame, text="Current File:").grid(
            row=0, column=0, sticky="w", pady=(0, 5)
        )
        ttk.Label(
            frame, textvariable=current_file_var, font=("Helvetica", 10, "bold")
        ...
        progress_bar = ttk.Progressbar(
            frame, variable=progress_var, maximum=total_submissions, length=300
        )
        ...
        ttk.Label(frame, textvariable=count_var).grid(
            row=2, column=1, sticky="e", pady=(5, 15)
        )

        # Add status message
        status_var = tk.StringVar(value="Starting...")
        ttk.Label(frame, textvariable=status_var, wraplength=350).grid(
            ...
            try:
                ...
                try:
                    ...
                    self.show_error_and_close(
                        progress_window, f"Invalid temperature value: {str(e)}"
                    )
                    return
                ...
                if not os.path.exists(system_prompt_path):
                    self.show_error_and_close(
                        progress_window, "System prompt file not found."
                    )
                    return

                if not os.path.exists(user_prompt_path):
                    self.show_error_and_close(
                        progress_window, "User prompt file not found."
                    )
                    return
                ...
                # Read prompts
                try:
                    system_prompt = self.document_processor.read_text_file(
                        system_prompt_path
                    )
                    user_prompt = self.document_processor.read_text_file(
                        user_prompt_path
                    )
                except Exception as e:
                    self.show_error_and_close(
                        progress_window, f"Failed to read prompts: {str(e)}"
                    )
                    return
                ...
                if output_folder:
                    try:
                        ...
                        self.show_error_and_close(
                            progress_window, f"Failed to create output folder: {str(e)}"
                        )
                        return
                ...
                for i, index in enumerate(selected_indices):
                    ...
                    self.update_progress_ui(
                        count_var, f"{i}/{total_submissions} completed"
                    )
                    self.update_progress_ui(progress_var, i)
                    self.update_progress_ui(
                        status_var,
                        f"Grading {filename}... (connecting to {self.string_vars['base_url'].get()})",
                    )
                    ...
                    try:
                        ...
                        if success:
                            success_count += 1
                            self.update_progress_ui(
                                status_var, f"Successfully graded {filename}"
                            )
                        else:
                            fail_count += 1
                            self.update_progress_ui(
                                status_var, f"Failed to grade {filename}: {feedback}"
                            )
                    except Exception as e:
                        fail_count += 1
                        self.update_progress_ui(
                            status_var, f"Error grading {filename}: {str(e)}"
                        )

                # Final update
                self.update_progress_ui(progress_var, total_submissions)
                self.update_progress_ui(
                    count_var, f"{total_submissions}/{total_submissions} completed"
                )
                ...
                self.show_error_and_close(
                    progress_window, f"Unexpected error: {str(e)}"
                )
    ...
    def grade_all(self):
        ...
        progress_var = tk.IntVar(value=0)
        progress_bar = ttk.Progressbar(
            frame, variable=progress_var, maximum=file_count, length=300
        )
        ...
        ttk.Label(frame, textvariable=count_var).grid(
            row=3, column=1, sticky="e", pady=(5, 15)
        )
        ...
        def run_grading():
            try:
                ...
                for i, filename in enumerate(filenames):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            fail_count += 1
                            self.update_progress_ui(
                                status_var, f"Failed to grade {filename}"
                            )
                    except Exception as e:
                        fail_count += 1
                ...
                self.update_progress_ui(
                    count_var, f"{file_count}/{file_count} completed"
                )
                self.update_progress_ui(status_var, "Finalizing...")
